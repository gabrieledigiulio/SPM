...
static std::vector<std::uint32_t> regular_nnz_pattern(std::size_t n,
...
static std::vector<std::uint32_t> weighted_nnz_pattern(std::size_t n,
                                                       std::uint64_t nz_total) {
  if (nz_total < n) {
  ...
  for (std::size_t i = 0; i < n; ++i) {
    const long double ideal =
        static_cast<long double>(extras_total) * weights[i] / sum_w;
  ...
  if (left != 0) {
    throw std::runtime_error(
        "internal error: could not distribute all nonzeros");
  }
...
static GeneratedMatrix generate_matrix(std::size_t n, std::uint64_t nz_total,
                                       std::uint64_t seed,
                                       const std::string &mode) {
  GeneratedMatrix G;
  G.A.n = n;

  const std::vector<std::uint32_t> nnz_per_row =
      make_nnz_pattern(n, nz_total, mode);
  ...
  if (prefix != nz_total) {
    throw std::runtime_error(
        "internal error: generated nnz does not match requested nz");
  }
...
static void print_matrix_stats(const GeneratedMatrix &G) {
  ...
  std::cout << "avg_nnz_per_row="
            << static_cast<double>(G.stats.total_nnz) /
                   static_cast<double>(G.A.n)
            << "\n";
}
