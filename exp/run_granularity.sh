...
echo "$SEP"
echo " GRANULARITY SWEEP ($REPEATS REPEATS)"
echo "$SEP"
...
for b in "${SPMV_CHUNK[@]}"; do
    ...
    for r in $(seq 1 "$REPEATS"); do
        ...
        out_cpp=$(srun --time="$SRUN_TIME" -N 1 -n 1 --cpus-per-task="$CPP_THREADS" \
            "$CPPTHREADS_BIN" -n "$N" -nz "$NZ" -m "$MODE" -s "$SEED" -t "$CPP_THREADS" -c "$b" -nc "$b")
        ...
        out_omp=$(srun --time="$SRUN_TIME" -N 1 -n 1 --cpus-per-task="$OMP_THREADS" \
            "$OMP_TASKS_BIN" -n "$N" -nz "$NZ" -m "$MODE" -s "$SEED" -t "$OMP_THREADS" -c "$b" -nc "$b")
        ...
        out_mpi=$(OMP_NUM_THREADS="$MPI_THREADS" srun --time="$SRUN_TIME" --mpi=pmix -N "$MPI_NODES" -n "$MPI_RANKS" \
            --cpus-per-task="$MPI_THREADS" "$MPI_OMP_BIN" -n "$N" -nz "$NZ" -m "$MODE" -s "$SEED" -t "$MPI_THREADS" -c "$b" -nc "$b")
