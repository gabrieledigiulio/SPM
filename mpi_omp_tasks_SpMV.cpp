...
// The functions operate on a subsection [begin, begin+count) of a vector
// not on the entire vector: each rank physically holds only its own portion
...
static double local_dot_omp_tasks(const std::vector<double>& a,
    ...
    {
        for (std::size_t off = 0; off < count; off += chunk_size) { // an offset variable off ranging from 0 to count is used, and then translated into absolute indices by adding begin
            const std::size_t start = begin + off; // the absolute start index of this chunk
            const std::size_t end = begin + std::min(off + chunk_size, count); // the absolute end index
            // one task per chunk:
...
// The scale factor is already calculated and passed in from the outside 
// rather than being derived from an internally calculated norm 
// The normalization factor must be calculated based on the global norm
...
static void local_scale_omp_tasks(std::vector<double>& v,
    ...
    {
        for (std::size_t off = 0; off < count; off += chunk_size) { // from offset by chunk_size
            ...
            {
                for (std::size_t i = start; i < end; ++i) {
                    v[i] *= factor; // scale each element in this chunk
                }
            }
        }
    } 
...
static void spmv_local_omp_tasks(const LocalMatrix& L,
    ...
    for (std::size_t start = 0; start < L.num_rows; start += chunk_size) { // iter directly from 0 to num_rows
        const std::size_t end = std::min(start + chunk_size, L.num_rows); // prevents last chunk extending

        // one task per chunk:
        // firstprivate(start, end) > each task gets its own copy
        // shared(L, x_full, y_phys) > tasks read L/x and write y on disjoint ranges 
...
static void apply_shift_permutation_omp_tasks(const std::vector<double>& y_phys_full,
    ...
    #pragma omp taskgroup
    {
        for (std::size_t start = 0; start < n; start += chunk_size) {
            ...
            {
                for (std::size_t p = start; p < end; ++p) { // iter over the vector y_phys_full
...
iterative_spmv_evolving_mpi_omp(const LocalMatrix& L, std::size_t n,
    ...
    std::vector<double> y_phys(L.num_rows); // local SpMV output indexed by phy row
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp single
        {
            ...
            // normalize x
            // x_full is already identical and complete on every
            // rank, so the reductionv below is purely local
            ...
            for (std::uint32_t iter = 0; iter < NUM_ITERS; ++iter) {
                ...
                spmv_local_omp_tasks(L, x_full, y_phys, chunk_size); // call the SPMV creates tasks, does not wait
                ...
                // local sum of squares, on this rank's own physical slice.
                // y_phys is distributed across ranks, so
                // this only produces a partial contribution to the global norm
                ...
                // MPI_Allreduce combines a value from all ranks using an operation
                ...
                // nevery rank knows the correct global norm can calculate the normalization factor
                const double inv = 1.0 / std::sqrt(sumsq_global);

                // each rank scales only its own local slice by the newly calculated global factor
                ...
                // apply the shift and then swap x_full with x_next
            ...
            // The final permutation and the rayleigh/checksum computation are pure
            // local work we can restrict them to rank 0 only
            if (rank == 0) {
                ...
                result.rayleigh = local_dot_omp_tasks(x_full, x_next, 0, n, norm_chunk_size);  // x · (A_shifted * x)
